...
- [Deno](https://deno.com/) 2.3+（推荐）或 Node.js 22+，并加入 `PATH`
...
YouTube 需要 JavaScript 运行时解决平台挑战；项目会优先使用 Deno，没有 Deno 时
自动使用 Node.js。
...
YouTube 会轮换已在普通浏览器标签页中使用的账号 Cookie。年龄限制等必须登录的
视频，建议按以下方式导出：

1. 新开一个无痕窗口，只保留一个标签页并登录 YouTube。
2. 在同一标签页打开 `https://www.youtube.com/robots.txt`。
3. 使用扩展导出 `youtube.com` 的 Netscape Cookie。
4. 导出后立即关闭整个无痕窗口，不要再次打开该无痕会话。
...
YouTube 的 JavaScript 挑战未能解析。安装 Deno 2.3+ 或 Node.js 22+，并重新执行
`python -m pip install -r requirements.txt` 安装 `yt-dlp-ejs`。

### `cookies are no longer valid`

YouTube 已轮换 Cookie。按上面的无痕窗口与 `robots.txt` 步骤重新导出。

### B站课程提示 DRM 加密

部分 `cheese/play` 课程使用 DRM 加密媒体流，即使账号有播放权限，yt-dlp 和 FFmpeg
也无法解密合并。程序会读取媒体文件头进行预检，确认加密后在完整下载前停止。
请改用 B站官方缓存功能或联系课程提供方获取允许下载的资源。
