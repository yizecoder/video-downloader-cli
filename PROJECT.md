...
2026-06-22 对 `_playground/kimi-playground` 做了退役评估。正式项目吸收以下体验：

- URL 文本文件批量导入、注释/空行处理和顺序去重。
- B站视频链接跟踪参数清理，同时保留分 P 参数。

未迁移本地视频转写、模型下载、Excel 下载记录、日期子目录、自动虚拟环境切换和
捆绑 FFmpeg。这些功能会扩大依赖、引入平台特例，或与当前统一 CLI 的维护边界冲突。
